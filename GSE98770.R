#---------------------------------------Figure4 A: Boxplot of the hub genes in GSE98770----------------------------------------------------------------------
setwd('D:/科研工作站/生信工作站/My project/P003/GSE98770/deg')
deg <- readRDS(file = 'deg/deg.rds')
md <- readRDS(file = 'deg/md.rds')
#hub genes
...
exp.hub <- pivot_longer(exp.hub,cols = 2:12,names_to = 'sample',values_to = 'value')
exp.hub$group <- rep(md$Group,5)
ggplot(data = exp.hub,aes(x=symbol,y=value,fill=group))+geom_boxplot()+theme_classic()+
  theme(plot.title = element_text(hjust = 0.5),legend.position = 'top')+xlab('')+ylab('Relative expression')+ggtitle('GSE98770')
ggsave(filename = 'boxplot-hubgene.pdf',width = 6,height = 3.5)
#---------------------------------------Figure4 C: ROC of the hub genes in GSE98770--------------------------------------------------------------------------
setwd('D:/科研工作站/生信工作站/My project/P003/GSE98770/roc')
library(pROC)
